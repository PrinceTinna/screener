...
def calculate_net_irr(pf, mtf_pct=9.7, tax_pct=20.0, return_breakdown=False):
    """
    Calculates True Net IRR by deducting pro-rated MTF interest and STCG Tax.
    """
    ...
    net_irr = np.median(final_returns) if len(final_returns) > 0 else 0.0
    
    if return_breakdown:
        tax_impact = np.median(np.where(post_mtf_returns > 0, post_mtf_returns * (tax_pct/100.0), 0))
        pre_tax_val = net_irr + tax_impact
        ...
        breakdown = {
            ...
            "Final Net Median Return (%)": float(net_irr * 100)
        }
        return net_irr, breakdown
